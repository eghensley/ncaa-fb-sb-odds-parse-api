...
import com.ehens86.bet.ncaa_fb_sb_odds_parse_api.service.TeamService;
...
public class TeamController {
	...
	@ApiOperation(value = "Get team object by ID")
	@GetMapping("/{teamId}")
	public ResponseEntity<GetResponse> getTeam(
			@RequestHeader(value = "password", required = true) String attemptedPassword,
			@RequestHeader(value = "debug", required = false) String debug,
			@RequestHeader(value = "stack", required = false) String stack, @PathVariable("teamId") String teamId) {
		...
		if (loginKey.equals(attemptedPassword)) {
			GetResponse response = teamsService.fetchTeam(teamId);
